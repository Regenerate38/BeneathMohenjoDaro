...
export class Room0 extends Scene {
  ...
  create() {
    ...
    const tileset = roomZeroTileMap.addTilesetImage(
      "room_zero_tiles",
      "room0tiles"
    );
    // console.log(startingRoomTileMap.layers.length.toString());
    for (let i = 0; i < roomZeroTileMap.layers.length; i++) {
      const layer = roomZeroTileMap.createLayer(i, "room_zero_tiles", 0, 0);
      layer.setDepth(i);
      layer.scale = 3;
    }
  ...
  update(_time, delta) {
    ...
    if (
      this.player.getTilePos().x === 14 &&
      this.player.getTilePos().y === 19 &&
      this.gridPhysics.facingDirection === Direction.DOWN
    )
